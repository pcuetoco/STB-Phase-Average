classdef BinnedStbDataLight < matlab.mixin.Copyable
    ...
    properties
        ...
        UprimeWprimeArray
        VprimeWprimeArray
        zoneName %Name of zone in Tecplot
    end
    
    methods
        function obj = BinnedStbDataLight(binCentroidXVector,...
                ...
                vprimeWprimeArray,zoneName,binNondimensionalTimeVector)
            ...
            for i=length(obj):-1:1
                ...
                % averaged flow
                    ...
                    % Set zone name with the nondimensional time instant
                     tdata.cubes.zonename = obj(1).zoneName;
                    % Set solution time of the zone
                    if (~isempty(nondimensionalTimeVector))
                    tdata.cubes(i).solutiontime =...
                        nondimensionalTimeVector(i);
                    else
                      tdata.cubes(i).solutiontime =0;
                ...
                % Grid coordinates
                tdata.cubes(i).x = obj(i).Xarray.*1e3; % x
                tdata.cubes(i).y = obj(i).Yarray.*1e3; % y
                tdata.cubes(i).z = obj(i).Zarray.*1e3; % z
                % Mean velocity
