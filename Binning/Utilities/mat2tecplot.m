...
% 
% Version # 1.7
% Last Update: Wen Long, Seattle, 08/10/2018
%
% Future work: 1) zone sharing, 2) face sharing to reduce datasize
%  3)support to read binary data out for manipulation
% 
...
if(~have_zonename)  %if no zone name, give one by default
   ...
   zone_name = ['Cube' int2str(icube)];  
    ...
    if(isstring(tdata.cubes(icube).zonename))
         tdata.cubes(icube).zonename=convertStringsToChars(tdata.cubes(icube).zonename);
    end
...
%     end
	...
	%check if varloc is available
        ...
        surface_varloc=zeros([tdata.Nvar,1]);
        for iv=1:tdata.Nvar
        ...
        switch(IsBlock)
            ...
            case 1  %block 
					...
					%
					%padding is in I direction, no padding in J direction (J varies slow,I varies fast)
					%

					surface_data_value=zeros([Imax,Jmax-1]); %define data so that (Imax:Imax,:) is 
										 %filled with zeros as padding
                                        ...
                                        %occupies the same size as nodal variables
			
					%NOTE the BELOW PADDING IS WRONG, WE SHOULD ONLY PAD at Imax:Imax 
					...
					%
                                        %padding is in J direction, no padding in K direction (K varies slow,J varies fast)
                                        %

                                        surface_data_value=zeros([Jmax,Kmax-1]); %define data so that (Jmax:Jmax,:) is 
                                                                                 %filled with zeros as padding
					...
					%NOTIE THE FOLLWING PADDING IS WRONG, WE SHOULD ONLY PAD at JMAX:JMAX
					...
					%
                                        %padding is in I direction, no padding in K direction (K varies slow,I varies fast)
                                        %

                                        surface_data_value=zeros([Imax,Kmax-1]); %define data so that (Imax:Imax,:) is 
                                                                                 %filled with zeros as padding
					...
					%NOTIE THE FOLLOWING PADDING IS WRONG, WE SHOULD ONLY PAD AT Imax:Imax locations
...
% 	 end
	 ...
	 NV=tdata.Nvar;
         cube_varloc=zeros([tdata.Nvar,1]);
		...
		if(iv==1||iv==2||iv==3)
         ...
         switch(IsBlock)
             ...
             case 1 %IJK ordered data with block storage
				...
				%pad zeros here (NOTE: padding is only in I and J direction, not in K direction)
                                cube_data_value=zeros([Imax, Jmax,Kmax-1]); %padding by making Imax:Imax, Jmax:Jmax zero
									    %upfront
					...
					cube_data_value=zeros([Imax, Jmax,Kmax-1]);
					cube_data_value(1:Imax-1,1:Jmax-1,1:Kmax-1)=tdata.cubes(icube).v(iv-3,...
											1:Imax-1,1:Jmax-1,1:Kmax-1);
                                end

				Imax_v=size(cube_data_value,1);
                                Jmax_v=size(cube_data_value,2);
                                Kmax_v=size(cube_data_value,3);	%note that Kmax_v = Kmax-1 for cell-centered

				%cube_data_value=reshape(cube_data_value,[1,Imax_v*Jmax_v*Kmax_v]);
                                %also need to pad zeros to make sure it (only needed for ordered data)
                                %occupies the same size as nodal variables
				%NOTICE: THE FOLLOWING PADDING IS WRONG, IT SHOULD BE ONLY PADDING AT Imax:Imax Jmax:Jmax
				%locations
...
%            +-----------+
	...
	if(~have_varloc) %give default value as 0 (nodal)
	   ...
	   NV=tdata.Nvar;
           FEline_varloc=zeros([tdata.Nvar,1]);
	   for iv=1:NV   %NV is number of variables 
 	       if(~have_varloc)
                  ...
                  for iv=1:tdata.Nvar
                      switch(FEline_varloc(iv))
                        ...
                        case 1  %cell center
                             ...
                             %%also need to pad zeros to make sure it  (only needed for ordered data)
                             %%occupies the same size as nodal variables
			     %WLONG: DO WE PADD FOR FE-lines at all?
                  ...
                  for iv=1:tdata.Nvar
                        switch(FEsurface_varloc(iv))
                            ...
                            case 1  %cell center
                                 ...
                                 %%also need to pad zeros to make sure it  (only needed for ordered data)
                                 %%occupies the same size as nodal variables
				 %WLONG: Do we pad for FEsurfaces at all?
...
%     end
    ...
    %give location for each variable in this zone
    %
    FEvol_varloc=zeros([tdata.Nvar,1]);
    for iv=1:tdata.Nvar %NV is number of variables
